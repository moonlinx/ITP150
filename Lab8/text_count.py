...
def count_words(fileHandle):
    fileHandle = open('declaration.txt')
    ...
    for line in lst:
        ...
        line = line.replace('.', '').replace(',', '').replace('!', '') # replace characters
    ...
    sortedDict = sorted(word_count.items(),
                        key = lambda x: x[1],
                        reverse=True)

    fileHandle = open('declaration.txt', 'w')
    for word, count in sortedDict:
        fileHandle.write(f"{word}: {count}\n")
    fileHandle.close()

count_words('declaration.txt')
